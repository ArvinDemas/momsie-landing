 ...
 */
export const onWithdrawalCreate = functions.firestore
 ...
 */
export const onWithdrawalUpdate = functions.firestore
  .document('withdrawals/{wId}')
 ...
 */
export const onSopSubmissionCreate = functions.firestore
 ...
 */
export const expireUnclaimedBookings = functions.pubsub
...
function getAuthHeader(isProd: boolean): string {
  const key = isProd ? PROD_SERVER_KEY : SANDBOX_SERVER_KEY;
  if (!key) throw new functions.https.HttpsError('internal', 'Midtrans server key not configured');
...
export const onBookingCompleted = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    const doulaUid = booking.doulaUid as string;
    const doulaEarnings = booking.doulaEarnings as number;
    const bookingId = context.params.bookingId;

    if (!doulaUid || doulaEarnings == null || doulaEarnings <= 0) return null;
    ...
    try {
      ...
      await db.runTransaction(async (transaction) => {
        ...
        const data = snap.data()!;
        const currentEscrow = (data['saldo_escrow'] as number) ?? 0;
        const currentTersedia = (data['saldo_tersedia'] as number) ?? 0;
        const currentTotal = (data['totalPendapatan'] as number) ?? 0;
      ...
      functions.logger.info(
        `Booking ${bookingId} completed. Added Rp${doulaEarnings} to doula ${doulaUid}.`
      );
    } catch (e) {
 ...
 */
export const createMidtransSnap = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  try {
    const response = await fetch(
      `https://app.${isProd ? '' : 'sandbox.'}midtrans.com/snap/v1/transactions`,
      {
        method: 'POST',
        headers: {
 ...
 */
export const checkMidtransStatus = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User harus login');
  }
  const isProd = data.isProduction === true;
  const { orderId } = data;
  if (!orderId) throw new functions.https.HttpsError('invalid-argument', 'orderId wajib diisi');

  try {
    const response = await fetch(
      `https://api.${isProd ? '' : 'sandbox.'}midtrans.com/v2/${orderId}/status`,
      {
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
